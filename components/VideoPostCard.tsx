...
function buildVideoCandidates(
  ...
  const editedFromSource = rawSourceVideo
    ? buildCloudinaryEditedUrl(rawSourceVideo, { cropPreset, filterPreset })
    : "";
...
export default function VideoPostCard({
  ...
  // IMPORTANT: keep absolute fill so contain works consistently
  const videoStyle: any = useMemo(() => {
    if (isWeb) return [StyleSheet.absoluteFillObject, { objectFit: "contain" }];
  ...
  const onVideoError = useCallback(() => {
    ...
    setVideoSourceIndex((prev) => {
      const next = prev + 1;
      return next >= videoCandidates.length ? prev : next;
    });
  ...
  useEffect(() => {
    ...
    if (isActive && canPlayVideo && activeVideoUrl) {
      if (clipStartMs > 0) {
        video.setPositionAsync(clipStartMs).then(() => video.playAsync()).catch(() => null);
      } else {
          ...
          {canPlayVideo ? (
            <View style={styles.actionItem}>
              <Pressable
                ...
                onPress={() => onToggleMute().catch(() => null)}
              >
